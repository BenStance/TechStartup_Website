...
const ClientLayout = () => {
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      // Search client's projects
      try {
        const projectsResponse = (await projectsApi.getAllProjects()).data;
        const clientProjects = projectsResponse.filter(project => 
          project.clientId === user?.id // Only projects owned by this client
        );
        
        const projectMatches = clientProjects.filter(project => 
        ...
        projectMatches.forEach(project => {
          searchResults.push({
            ...
            path: `/dashboard/client/projects/${project.id}`,
            type: 'project'
          });
        });
      } catch (error) {
  ...
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: <Home className="w-5 h-5" />, path: '/dashboard/client' },
    { id: 'projects', label: 'My Projects', icon: <Briefcase className="w-5 h-5" />, path: '/dashboard/client/projects' },
    { id: 'notifications', label: 'Notifications', icon: <BellIcon className="w-5 h-5" />, path: '/dashboard/client/notifications' },
    { id: 'uploads', label: 'Uploads', icon: <Upload className="w-5 h-5" />, path: '/dashboard/client/uploads' },
  ];

  // Quick actions for client role
  const quickActions = [
    { icon: <Plus className="w-4 h-4" />, label: 'New Project', action: () => navigate('/dashboard/client/projects/create') },
    { icon: <Upload className="w-4 h-4" />, label: 'Upload File', action: () => navigate('/dashboard/client/uploads') },
    { icon: <BellIcon className="w-4 h-4" />, label: 'Check Notifications', action: () => navigate('/dashboard/client/notifications') },
  ];

  // Client notifications
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'Project Update', message: 'Your project has been approved', time: '5 min ago', type: 'info', unread: true },
    { id: 2, title: 'Requirement Received', message: 'We have received your requirements', time: '1 hour ago', type: 'success', unread: true },
    { id: 3, title: 'Project Completed', message: 'Your project has been completed', time: '2 hours ago', type: 'success', unread: false },
    { id: 4, title: 'File Uploaded', message: 'Your requirements document has been uploaded', time: '3 hours ago', type: 'info', unread: false },
    { id: 5, title: 'Progress Update', message: 'Your project is 75% complete', time: '4 hours ago', type: 'info', unread: false },
  ]);
  ...
  const handleNotificationClick = (notification) => {
    ...
    } else if (notification.title.includes('Requirement')) {
      navigate('/dashboard/client/projects');
    } else if (notification.title.includes('File')) {
      navigate('/dashboard/client/uploads');
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden md:block text-left">
                    ...
                    <div className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Project Client</div>
                  </div>
            ...
            {sidebarOpen && (
              ...
              >
                ...
                <div className={`text-xs ${
                  ...
                  My Projects: {notifications.filter(n => n.type !== 'error').length}
                </div>
