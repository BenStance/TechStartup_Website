...
import { 
  Plus, Eye, Edit, Search, Filter, MoreVertical, 
...
const ControllerProjects = () => {
  ...
  // Load projects assigned to this controller
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        setLoading(true);
        // Fetch projects assigned to this controller (filtered by backend based on JWT role)
        const projectsData = await projectsApi.getAllProjects();
        
        // Filter projects assigned to this controller as a fallback (backend should handle this)
        const controllerProjects = projectsData.filter(project => 
          project.controllerId === user?.id
        );
        
        // Convert snake_case fields to camelCase to match frontend expectations
        const convertedProjects = controllerProjects.map(project => ({
          ...project,
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className={`mt-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              Manage and track projects assigned to you
            </p>
            <div className="flex items-center gap-4 mt-4">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-sm">{projects.length} total projects assigned</span>
              </div>
              <div className="flex items-center gap-2">
      ...
      <div className={`p-6 rounded-2xl border backdrop-blur-sm ${theme === 'dark'
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          {[
            { label: 'Active', count: projects.filter(p => ['planning', 'designing', 'development', 'testing', 'delivery', 'in_progress'].includes(p.status)).length, color: 'bg-blue-500' },
        ...
        {filteredProjects.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className={`mb-6 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              {searchTerm ? 'Try adjusting your search or filters' : 'You have no assigned projects yet'}
            </p>
          </div>
        ) : (
