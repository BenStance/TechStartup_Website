    ...
    const fetchProject = async () => {
      try {
        ...
        const convertedProject = {
          ...
          serviceName: projectData.service_name || projectData.serviceName,
        ...
        // Fetch related data in parallel - only fetch client and controller, not service since it's already in the response
        const [clientData, controllerData] = await Promise.all([
                ...
                <div className="space-y-4">
                  {(project.amount !== undefined && project.amount !== null) && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Amount (TZS)</p>
                        <p className="font-medium">TZS {project.amount.toLocaleString()}</p>
                      </div>
                    </div>
                  )}
