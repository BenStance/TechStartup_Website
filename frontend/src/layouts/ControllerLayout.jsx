...
import { 
  ...
  TrendingUp, Upload, Folder, Receipt
} from 'lucide-react';
...
const ControllerLayout = () => {
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      // Search projects assigned to controller
      try {
        const projectsResponse = (await projectsApi.getAllProjects()).data;
        const controllerProjects = projectsResponse.filter(project => 
          project.controllerId === user?.id // Only projects assigned to this controller
        );
        
        const projectMatches = controllerProjects.filter(project => 
        ...
        projectMatches.forEach(project => {
          searchResults.push({
            ...
            path: `/dashboard/controller/projects/${project.id}`,
            type: 'project'
          });
        });
      } catch (error) {
      ...
      try {
        ...
        const clientMatches = usersResponse.filter(user => 
          user.role === 'client' &&
        ...
        clientMatches.forEach(user => {
          searchResults.push({
            title: `${user.firstName} ${user.lastName}`,
            description: `Client - ${user.email}`,
            path: `/dashboard/controller/clients/${user.id}`,
            type: 'user'
          });
        });
      } catch (error) {
  ...
  // Navigation items for controller role
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: <Home className="w-5 h-5" />, path: '/dashboard/controller' },
    { id: 'projects', label: 'My Projects', icon: <Briefcase className="w-5 h-5" />, path: '/dashboard/controller/projects' },
    { id: 'clients', label: 'My Clients', icon: <Users className="w-5 h-5" />, path: '/dashboard/controller/clients' },
    { id: 'services', label: 'Services', icon: <Server className="w-5 h-5" />, path: '/dashboard/controller/services' },
    // { id: 'reports', label: 'Reports', icon: <BarChartIcon className="w-5 h-5" />, path: '/dashboard/controller/reports' },
    { id: 'selling', label: 'Selling', icon: <ShoppingBag className="w-5 h-5" />, path: '/dashboard/controller/selling' },
    { id: 'sales-history', label: 'Sales History', icon: <Receipt className="w-5 h-5" />, path: '/dashboard/controller/selling/history' },
    { id: 'uploads', label: 'Uploads', icon: <Upload className="w-5 h-5" />, path: '/dashboard/controller/uploads' },
    { id: 'notifications', label: 'Notifications', icon: <BellIcon className="w-5 h-5" />, path: '/dashboard/controller/notifications' },
  ];

  // Quick actions for controller role
  const quickActions = [
    { icon: <Plus className="w-4 h-4" />, label: 'New Project', action: () => navigate('/dashboard/controller/projects/create') },
    { icon: <ShoppingBag className="w-4 h-4" />, label: 'Sell Product', action: () => navigate('/dashboard/controller/selling') },
    { icon: <Receipt className="w-4 h-4" />, label: 'Sales History', action: () => navigate('/dashboard/controller/selling/history') },
    { icon: <Upload className="w-4 h-4" />, label: 'Upload File', action: () => navigate('/dashboard/controller/uploads') },
    { icon: <TrendingUp className="w-4 h-4" />, label: 'Update Progress', action: () => navigate('/dashboard/controller/projects') },
    { icon: <BellIcon className="w-4 h-4" />, label: 'Send Notification', action: () => navigate('/dashboard/controller/notifications/create') },
  ];

  // Mock notifications
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'Project Update Required', message: 'Website Redesign project needs progress update', time: '5 min ago', type: 'warning', unread: true },
    { id: 2, title: 'New Client Message', message: 'Acme Corp sent a new message about your project', time: '1 hour ago', type: 'info', unread: true },
    { id: 3, title: 'Project Completed', message: 'Mobile App Development project has been completed', time: '2 hours ago', type: 'success', unread: false },
    { id: 4, title: 'File Uploaded', message: 'Client uploaded new requirements document', time: '3 hours ago', type: 'info', unread: false },
    { id: 5, title: 'Deadline Approaching', message: 'Database Optimization project deadline in 2 days', time: '4 hours ago', type: 'error', unread: false },
  ]);
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.type === 'info' && notification.title.includes('Project')) {
      navigate('/projects');
    } else if (notification.title.includes('Client')) {
      navigate('/clients');
    } else if (notification.title.includes('File')) {
      navigate('/uploads');
            ...
            <div className="flex items-center">
              ...
              <div className="flex items-center ml-4 md:ml-0">
                ...
                <div className="ml-3">
                  ...
                  <div className="hidden sm:block text-xs opacity-70">Project Management Dashboard</div>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className="hidden md:block text-left">
                    <div className="text-sm font-medium">{user?.firstName || 'Controller'} {user?.lastName || ''}</div>
                    <div className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Project Controller</div>
                  </div>
            ...
            {sidebarOpen && (
              ...
              >
                ...
                <div className={`text-xs ${
                  ...
                  Assigned: {notifications.filter(n => n.type !== 'error').length}
                </div>
